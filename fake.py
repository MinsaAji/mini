...
# Read the datasets
train = pd.read_csv(r"C:\Users\minsa\OneDrive\Desktop\MINI PROJECT\mini\train.csv")
test = pd.read_csv(r"C:\Users\minsa\OneDrive\Desktop\MINI PROJECT\mini\test.csv")
...
def url_enter(url):
    ...
    slist = ["bbc","cnn","nbcnews","cbsnews","npr","apnews","reuters","pbs","theguardian","nytimes","timesofindia","thehindu","hindustantimes","indianexpress","ndtv"
,"wsj","washingtonpost","economist","newyorker","bloomberg","theatlantic","deccanchronicle"]
    
    if len(source_name_parts) > 2 and source_name_parts[0] == "edition":
        # If the first part is "edition", consider the next part as the source name
        source_name = source_name_parts[1]
    else:
        # Otherwise, use the first part as the source name
        source_name = source_name_parts[0]
    if source_name in slist:
